...
{
  ImmutableNodeContext* context = (ImmutableNodeContext*)calloc(1L, sizeof(ImmutableNodeContext));
 ...
 *
 * Parses an IO object, returning a native tracemonkey parse tree.
 */
static VALUE parse_io(int argc, VALUE *argv, VALUE klass) {
  ...
  assert(context->pc = new JSCompiler(context->js) );
  ...
  assert(context->pc->init(chars,length,NULL,filenamez,(unsigned)linenumi));
  /*
  ...
  */
  ...
  context->node = context->pc->parse( JS_NewObject(context->js, NULL, NULL, NULL) );
  /*
 ...
 *   pn_unsed
 *
 * Returns the used flag as a bool
 */
static VALUE pn_used(VALUE self) {
  ImmutableNodeContext * ctx;
  Data_Get_Struct(self, ImmutableNodeContext, ctx);
  return ctx->node->pn_used ? Qtrue : Qfalse;
}
...
static VALUE pn_type(VALUE self) {
  ...
  switch(ctx->node->pn_type) {
    ...
    case TOK_SEQ: return ID2SYM(rb_intern("tok_seq"));
    
    case TOK_FORHEAD: return ID2SYM(rb_intern("tok_forhead"));
    
    case TOK_ARGSBODY: return ID2SYM(rb_intern("tok_argsbody"));
    
    case TOK_UPVARS: return ID2SYM(rb_intern("tok_upvars"));
...
static VALUE data_pn_extra(VALUE self) {
  ...
  return UINT2NUM((unsigned long)(ctx->node->pn_xflags));
}
...
static VALUE regexp(VALUE self) {
  ...
  Data_Get_Struct(self, ImmutableNodeContext, ctx);
  js_regexp_toString(ctx->js, ctx->node->pn_objbox->object, &result);
...
static VALUE function_name(VALUE self) {
  ...
  Data_Get_Struct(self, ImmutableNodeContext, ctx);
  object = ctx->node->pn_funbox->object;
  f = (JSFunction *)JS_GetPrivate(ctx->js, ctx->node->pn_funbox->object);
...
static VALUE function_body(VALUE self) {
  ...
  if(ctx->node->pn_body) {
    ...
    /* in some cases the args can be in the body node ...
       for now, throwing the args away, not sure if this should be combined with
       function_args
    */
    
    JSParseNode* body = ctx->node->pn_body;
    
    switch(body->pn_type) {
    case TOK_ARGSBODY:
      JSParseNode* list = body->pn_kid;
      /* skip the args; the body lc is the last element in the list */
      while(list->pn_next){
        list = list->pn_next;
      }
      body = list;
...
VALUE jsop_to_symbol(JSUint32 jsop)
{
  switch(jsop) {
    ...
    case JSOP_ITER: return ID2SYM(rb_intern("jsop_iter"));
    
    case JSOP_NEXTITER: return ID2SYM(rb_intern("jsop_nextiter"));
    
    case JSOP_ENDITER: return ID2SYM(rb_intern("jsop_enditer"));
    
    case JSOP_APPLY: return ID2SYM(rb_intern("jsop_apply"));
    ...
    case JSOP_IMACOP: return ID2SYM(rb_intern("jsop_imacop"));
    ...
    case JSOP_LAMBDA: return ID2SYM(rb_intern("jsop_lambda"));
    
    case JSOP_CALLEE: return ID2SYM(rb_intern("jsop_callee"));
    
    case JSOP_SETLOCALPOP: return ID2SYM(rb_intern("jsop_setlocalpop"));
    
    case JSOP_PICK: return ID2SYM(rb_intern("jsop_pick"));
    
    case JSOP_TRY: return ID2SYM(rb_intern("jsop_try"));
    
    case JSOP_FINALLY: return ID2SYM(rb_intern("jsop_finally"));
    
    case JSOP_GETDSLOT: return ID2SYM(rb_intern("jsop_getdslot"));
    
    case JSOP_CALLDSLOT: return ID2SYM(rb_intern("jsop_calldslot"));
    ...
    case JSOP_GETUPVAR: return ID2SYM(rb_intern("jsop_getupvar"));
    
    case JSOP_CALLUPVAR: return ID2SYM(rb_intern("jsop_callupvar"));
    ...
    case JSOP_IFPRIMTOP: return ID2SYM(rb_intern("jsop_ifprimtop"));
    
    case JSOP_PRIMTOP: return ID2SYM(rb_intern("jsop_primtop"));
    ...
    case JSOP_CALLBUILTIN: return ID2SYM(rb_intern("jsop_callbuiltin"));
    ...
    case JSOP_NEWARRAY: return ID2SYM(rb_intern("jsop_newarray"));
    
    case JSOP_HOLE: return ID2SYM(rb_intern("jsop_hole"));
    
    case JSOP_DEFFUN_FC: return ID2SYM(rb_intern("jsop_deffun_fc"));
    
    case JSOP_DEFLOCALFUN_FC: return ID2SYM(rb_intern("jsop_deflocalfun_fc"));
    
    case JSOP_LAMBDA_FC: return ID2SYM(rb_intern("jsop_lambda_fc"));
    
    case JSOP_OBJTOP: return ID2SYM(rb_intern("jsop_objtop"));
    
    case JSOP_LOOP: return ID2SYM(rb_intern("jsop_loop"));
    
    case JSOP_GETUPVAR_DBG: return ID2SYM(rb_intern("jsop_getupvar_dbg"));
    
    case JSOP_CALLUPVAR_DBG: return ID2SYM(rb_intern("jsop_callupvar_dbg"));
    
    case JSOP_DEFFUN_DBGFC: return ID2SYM(rb_intern("jsop_deffun_dbgfc"));
    
    case JSOP_DEFLOCALFUN_DBGFC: return ID2SYM(rb_intern("jsop_deflocalfun_dbgfc"));
    
    case JSOP_LAMBDA_DBGFC: return ID2SYM(rb_intern("jsop_lambda_dbgfc"));
...
void init_Johnson_TraceMonkey_Immutable_Node(VALUE tracemonkey)
{
  ...
  */
  ...
  /* ImmutableNode class. */
  cNode = rb_define_class_under(tracemonkey, "ImmutableNode", rb_cObject);
  ...
  rb_define_method(cNode, "pn_used", (ruby_callback)pn_used, 0);
