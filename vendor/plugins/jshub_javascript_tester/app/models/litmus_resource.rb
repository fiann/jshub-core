...
class LitmusResource < ActiveResource::Base
  self.site = "#{JSHUB_JAVASCRIPT_TESTER[:test_run][:vendor][:site]}"
  self.timeout = 10
  
  # credentials
  # TODO move to YAML file in config
  site.user = "#{JSHUB_JAVASCRIPT_TESTER[:test_run][:vendor][:user]}"
  site.password = "#{JSHUB_JAVASCRIPT_TESTER[:test_run][:vendor][:password]}"
