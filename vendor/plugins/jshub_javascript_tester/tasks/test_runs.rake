namespace :jshub do
  namespace :test_runs do
    ...
    task :latest => :environment do
      ...
      if JSHUB_JAVASCRIPT_TESTER[:test_run][:vendor][:url_prefix] != 'secret'
        # Ruby HTTP client
        ...
        url = URI.parse("#{JSHUB_JAVASCRIPT_TESTER[:test_run][:vendor][:url_prefix]}/#{JSHUB_JAVASCRIPT_TESTER[:test_run][:vendor][:name]}/external/latest")
        ...
        url = URI.parse("#{JSHUB_JAVASCRIPT_TESTER[:test_run][:vendor][:url_prefix]}/#{JSHUB_JAVASCRIPT_TESTER[:test_run][:vendor][:name]}/runs/#{@run_id.to_s}/results.xml")
        req = Net::HTTP::Get.new(url.path)
        res = Net::HTTP.start(url.host, url.port) {|http|
          http.request(req)
        }
